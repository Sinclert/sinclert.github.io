...
## What is Terraform?
My first approach to Terraform was back in January 2019, when I was working as a Data Engineer.
At that time, I was not very sure what it was, as it had already been put in place, 
and I had not been present in the discussion involving its usage.

Terraform is a **command line** tool that allow developers to communicate with _IaaS_
cloud providers, in order _manage_ resources within those providers. The Terraform team
usually refers to this _management_ of resources as both the **creation** of resources,
and the **evolution** of those resources over time, covering everything stage of their
lifecycle: creation, modification, and tear down.

The list of supported cloud providers, alongside their documentation are:
...
called _Hashicorp Configuration Language_ (HCL). This language allow developers
to define providers and resources from different cloud providers.

We could say it is _"cloud agnostic"_.
...
For instance, this is an SQL Database resource definition within GCP:
...
This is the **most important file** from all the ones involve in the tool usage.
...
set of resources. In short, it is the **source of truth** of has been created
within a cloud project.

If something is not in the state file, then it does not exist to Terraform.

Therefore, using Terraform involves:

1. Create / get the initial state: `terraform init`
2. Check the diff between the HCL files and the state: `terraform plan`
2. Apply the diff between the HCL files and the state: `terraform apply`


## Benefits of using Terraform
You may be wondering why such a dedication to define cloud resources, given that
the easiest way to manage them is by using the Web UI all these great providers offer.
...
### Change traceability
As everything that has a billing associated to it, it is important to know when
infrastructure changes occur, and by whom.
...
(and to `.tfstate` changes during the process).
...
resources changes are propagated to real world infrastructure.
...
That is all for now. Later on, that _secret_ will be used to declare an env. variable
called  `GOOGLE_APPLICATION_CREDENTIALS`, used to authenticate CLI tools with a GCP project,
...
so that they form a good architectural pattern. (Imagine a UI-Server-Database architectural pattern).
